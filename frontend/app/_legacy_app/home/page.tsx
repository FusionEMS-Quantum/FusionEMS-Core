  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-body text-[var(--color-text-secondary)] mt-1">Select the operational domain for your role.</p>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {cards.map(([label, href]) => (
          <Link key={href} href={href} className="bg-[var(--color-bg-panel)] border border-border-default chamfer-8 p-4 hover:border-brand-orange transition-colors">
            <div className="text-label uppercase tracking-widest text-[var(--color-text-muted)]">Module</div>
