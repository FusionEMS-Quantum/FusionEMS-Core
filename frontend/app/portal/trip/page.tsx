...
function DebtTable({
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <div className="flex items-center gap-4 bg-bg-panel border border-border-subtle chamfer-8 px-4 py-3">
          <div>
            <div className="text-2xl font-black text-text-primary">{filtered.length}</div>
            <div className="text-micro text-text-muted">debts ({statusFilter})</div>
          </div>
          <div className="w-px h-8 bg-border-subtle" />
          <div>
            <div className="text-2xl font-black text-yellow-400">{fmt$(totalBalance)}</div>
            <div className="text-micro text-text-muted">total balance</div>
          </div>
          <div className="w-px h-8 bg-border-subtle" />
          <div>
            <div className="text-2xl font-black text-blue-400">{filtered.filter(d => d.status === 'ENROLLED').length}</div>
            <div className="text-micro text-text-muted">enrolled</div>
          </div>
          <div className="w-px h-8 bg-border-subtle" />
          <div>
            ...
            <div className="text-micro text-text-muted">intercepted</div>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-bg-panel border border-border-subtle chamfer-8 overflow-hidden">
          <table className="w-full text-left">
            ...
            <tbody>
              {filtered.map(debt => (
                <tr key={debt.id} className="border-b border-border-subtle hover:bg-white/[0.02]">
                  <td className="px-4 py-3 text-sm font-semibold text-text-primary">{debt.debtor_name || '—'}</td>
                  <td className="px-4 py-3 text-xs font-mono text-text-muted">{debt.debtor_ssn_masked || '***-**-****'}</td>
                  <td className="px-4 py-3 text-xs font-mono text-text-secondary">{debt.debt_id || debt.id.slice(0, 12)}</td>
  ...
  return (
    <div className="bg-bg-panel border border-border-subtle chamfer-8 overflow-hidden">
      <table className="w-full text-left">
        ...
        <tbody>
          {exports.map(exp => (
            <tr key={exp.id} className="border-b border-border-subtle hover:bg-white/[0.02]">
              <td className="px-4 py-3 text-xs font-mono text-text-muted">{exp.id.slice(0, 12)}</td>
              <td className="px-4 py-3 text-xs font-mono text-text-muted">
                {exp.generated_at ? new Date(exp.generated_at).toLocaleString() : '—'}
              </td>
              <td className="px-4 py-3 text-sm text-text-secondary">{exp.record_count ?? '—'}</td>
              ...
              <td className="px-4 py-3 text-xs font-mono text-text-muted truncate max-w-xs">{exp.filename || '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Submitted', value: (data.total_submitted ?? 0).toString(), color: 'text-text-primary' },
          ...
          <div key={m.label} className="bg-bg-panel border border-border-subtle chamfer-8 px-4 py-4">
      ...
      <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
        <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Intercept Pipeline</div>
          ...
          {[
            ...
            return (
              <div key={stage.label} className="flex items-center gap-3">
                <span className="text-micro text-text-muted w-28 flex-shrink-0">{stage.label}</span>
...
function SettingsView({ settings, onSave }: { settings: TRIPSettings; onSave: (_s: TRIPSettings) => Promise<void> }) {
  ...
  return (
    <div className="max-w-lg space-y-4">
      ...
      <div className="bg-bg-panel border border-border-subtle chamfer-8 p-5 space-y-4">
        {[
          ...
          <div key={field.key}>
            <label className="text-micro uppercase tracking-widest text-text-muted block mb-1.5">{field.label}</label>
            <input
              type={field.type}
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <label htmlFor="auto_enroll" className="text-sm text-text-secondary">Auto-enroll eligible debts (90+ days, qualifying payer)</label>
        </div>
  ...
  return (
    <div className="flex flex-col bg-bg-void min-h-screen">
      ...
      <div className="flex-shrink-0 border-b border-border-subtle bg-bg-panel/50 px-5 py-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-black text-text-primary uppercase tracking-widest">Wisconsin TRIP</div>
            <div className="text-micro text-text-muted">Tax Refund Intercept Program · Debt enrollment · DOR exports · Reconciliation</div>
          </div>
          <div className="flex items-center gap-2">
            {candidateCount > 0 && (
        ...
        <div className="grid grid-cols-4 gap-3 mt-4">
          {[
            { label: 'Total TRIP Debts', value: debts.length.toString(), color: 'text-text-primary' },
