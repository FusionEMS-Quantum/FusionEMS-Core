...
const HYDRANT_COLORS: Record<string, { label: string; bg: string; border: string; text: string }> = {
  ...
  ORANGE: { label: 'Class A   (500-999 GPM)', bg: 'bg-orange-900/30', border: 'border-orange-500/50', text: 'text-orange-400' },
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <input
          ...
          className="flex-1 max-w-xs bg-bg-panel border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-brand-orange/60"
        />
        <button
      ...
      <div className="bg-brand-orange/5 border border-brand-orange/20 chamfer-8 p-3 flex items-start gap-3">
        <div className="w-6 h-6 rounded-full bg-brand-orange/20 flex items-center justify-center flex-shrink-0 mt-0.5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {filtered.map(plan => (
            <div
              ...
              className="bg-bg-panel border border-border-subtle chamfer-8 p-4 cursor-pointer hover:border-brand-orange/40 hover:bg-brand-orange/[0.02] transition-all group"
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="text-sm font-semibold text-text-primary group-hover:text-brand-orange transition-colors">
                    {plan.name}
                  </div>
                  <div className="text-micro text-text-muted mt-0.5">{plan.address}</div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-1 mt-3">
                <div className="text-micro text-text-muted">
                  Type: <span className="text-text-secondary">{plan.occupancy_type || '—'}</span>
                </div>
                <div className="text-micro text-text-muted">
                  Stories: <span className="text-text-secondary">{plan.stories ?? '—'}</span>
                </div>
                <div className="text-micro text-text-muted">
                  Construction: <span className="text-text-secondary">{plan.construction_type || '—'}</span>
                </div>
                <div className="text-micro text-text-muted">
                  Reviewed:{' '}
                  <span className={plan.last_reviewed_at ? 'text-green-400' : 'text-orange-400'}>
              ...
              {plan.notes && (
                <div className="mt-2 text-micro text-text-muted line-clamp-2">{plan.notes}</div>
              )}
            </div>
          ))}
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-bg-void border border-brand-orange/30 chamfer-16 p-6 w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-text-primary">New Pre-Incident Plan</h2>
              <button onClick={() => setShowForm(false)} className="text-text-muted hover:text-text-primary transition-colors">✕</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-micro text-text-muted mb-1">STRUCTURE NAME *</label>
                <input
              ...
              <div>
                <label className="block text-micro text-text-muted mb-1">ADDRESS *</label>
                <input
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-micro text-text-muted mb-1">OCCUPANCY TYPE</label>
                  <select
                ...
                <div>
                  <label className="block text-micro text-text-muted mb-1">CONSTRUCTION TYPE</label>
                  <select
              ...
              <div>
                <label className="block text-micro text-text-muted mb-1">STORIES</label>
                <input
              ...
              <div className="border border-border-subtle chamfer-4 p-3 space-y-2">
                <div className="text-micro text-text-muted uppercase tracking-wider mb-2">Fire Protection Systems</div>
                {[
              ...
              <div>
                <label className="block text-micro text-text-muted mb-1">TACTICAL NOTES</label>
                <textarea
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-bg-void border border-border-subtle chamfer-16 p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h2 className="text-base font-bold text-text-primary">{selected.name}</h2>
                <div className="text-sm text-text-muted">{selected.address}</div>
              </div>
              <button onClick={() => setSelected(null)} className="text-text-muted hover:text-text-primary transition-colors ml-4 flex-shrink-0">✕</button>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              {[
                ...
                <div key={k} className="bg-bg-panel border border-border-subtle chamfer-4 p-3">
                  <div className="text-micro text-text-muted uppercase tracking-wider">{k}</div>
                  <div className="text-sm font-semibold text-text-primary mt-1">{v}</div>
                </div>
              ))}
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                ...
                <div key={String(label)} className={`border chamfer-4 p-3 text-center ${active ? 'bg-green-900/20 border-green-500/30' : 'bg-bg-panel border-border-subtle'}`}>
                  <div className={`text-sm font-semibold ${active ? 'text-green-400' : 'text-text-muted'}`}>
                    {active ? '✓' : '✗'} {label}
                  </div>
                </div>
              ))}
            ...
            {selected.notes && (
              <div className="bg-bg-panel border border-border-subtle chamfer-4 p-3">
                <div className="text-micro text-text-muted uppercase tracking-wider mb-1">Tactical Notes</div>
                <div className="text-sm text-text-secondary whitespace-pre-wrap">{selected.notes}</div>
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="bg-bg-panel border border-border-subtle chamfer-8 p-3 text-center">
          <div className="text-2xl font-black text-text-primary">{hydrants.length}</div>
          <div className="text-micro text-text-muted uppercase tracking-wider">Total Hydrants</div>
        </div>
        <div className={`bg-bg-panel border chamfer-8 p-3 text-center ${outOfService > 0 ? 'border-red-500/30' : 'border-border-subtle'}`}>
          <div className={`text-2xl font-black ${outOfService > 0 ? 'text-red-400' : 'text-green-400'}`}>{outOfService}</div>
          <div className="text-micro text-text-muted uppercase tracking-wider">Out of Service</div>
        </div>
        <div className={`bg-bg-panel border chamfer-8 p-3 text-center ${needingTest > 0 ? 'border-orange-500/30' : 'border-border-subtle'}`}>
          <div className={`text-2xl font-black ${needingTest > 0 ? 'text-orange-400' : 'text-green-400'}`}>{needingTest}</div>
          <div className="text-micro text-text-muted uppercase tracking-wider">Test Overdue</div>
        </div>
        <div className="bg-bg-panel border border-border-subtle chamfer-8 p-3 text-center">
          <div className="text-2xl font-black text-green-400">{hydrants.length - outOfService}</div>
          <div className="text-micro text-text-muted uppercase tracking-wider">Operational</div>
        </div>
      </div>
                ...
                filter === f
                  ...
                  : f === 'OOS'
                    ...
                    : HYDRANT_COLORS[f]
                      ...
                      : 'border-border-subtle text-text-muted hover:border-brand-orange/30'
              }`}
            >
      ...
      <div className="bg-bg-panel border border-border-subtle chamfer-8 p-3">
        <div className="text-micro text-text-muted uppercase tracking-wider mb-2">ISO Color Classification</div>
          ...
          {Object.entries(HYDRANT_COLORS).map(([color, { label, bg, border, text }]) => (
            <div key={color} className={`${bg} border ${border} chamfer-4 p-2 flex items-center gap-2`}>
              <div className={`w-3 h-3 rounded-full ${
                color === 'RED' ? 'bg-red-500' : color === 'ORANGE' ? 'bg-orange-500' : color === 'GREEN' ? 'bg-green-500' : 'bg-blue-500'
              }`} />
              ...
              {filtered.map(h => {
                ...
                return (
                  <tr key={h.id} className="border-b border-border-subtle hover:bg-white/[0.02]">
                    <td className="px-4 py-3 text-sm font-bold text-text-primary">{h.hydrant_number}</td>
                    <td className="px-4 py-3 text-micro text-text-muted font-mono">
                    ...
                    <td className="px-4 py-3 text-sm text-text-secondary">{h.flow_rate_gpm ?? '—'}</td>
                    <td className="px-4 py-3 text-sm text-text-secondary">{h.static_pressure_psi ?? '—'}</td>
                    <td className="px-4 py-3 text-sm text-text-secondary">
                      {h.last_tested_at ? new Date(h.last_tested_at).toLocaleDateString() : (
                        <span className="text-orange-400">Never</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-sm text-text-secondary">{h.hydrant_type || '—'}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-bg-void border border-brand-orange/30 chamfer-16 p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-bold text-text-primary">Register Hydrant</h2>
              <button onClick={() => setShowForm(false)} className="text-text-muted hover:text-text-primary">✕</button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-micro text-text-muted mb-1">HYDRANT NUMBER *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-micro text-text-muted mb-1">LATITUDE *</label>
                  <input type="number" step="0.0001" value={form.latitude} onChange={e => setForm(f => ({ ...f, latitude: e.target.value }))}
                    className="w-full bg-bg-panel border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-brand-orange/60" />
                </div>
                <div>
                  <label className="block text-micro text-text-muted mb-1">LONGITUDE *</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-micro text-text-muted mb-1">FLOW RATE (GPM)</label>
                  <input type="number" value={form.flow_rate_gpm} onChange={e => setForm(f => ({ ...f, flow_rate_gpm: e.target.value }))}
                    className="w-full bg-bg-panel border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-brand-orange/60" />
                </div>
                <div>
                  <label className="block text-micro text-text-muted mb-1">STATIC PRESSURE (PSI)</label>
                  <input type="number" value={form.static_pressure_psi} onChange={e => setForm(f => ({ ...f, static_pressure_psi: e.target.value }))}
                    className="w-full bg-bg-panel border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-brand-orange/60" />
                </div>
              </div>
              <div>
                <label className="block text-micro text-text-muted mb-1">ISO COLOR CLASS</label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-text-secondary">Currently In Service</span>
              </label>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowForm(false)} className="flex-1 quantum-btn-sm py-2">Cancel</button>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          { label: 'Corrective Action', value: statusCounts.corrective, color: 'text-orange-400', border: 'border-orange-500/20' },
          { label: 'Failed', value: statusCounts.failed, color: 'text-red-400', border: 'border-red-500/20' },
        ].map(({ label, value, color, border }) => (
          <div key={label} className={`bg-bg-panel border ${border || 'border-border-subtle'} chamfer-8 p-3 text-center`}>
      ...
      {inspections.length === 0 ? (
        ...
        <div className="bg-bg-panel border border-border-subtle chamfer-8 overflow-hidden">
          <table className="w-full text-left">
            ...
            <tbody>
              {inspections.map(i => (
                <tr key={i.id} className="border-b border-border-subtle hover:bg-white/[0.02]">
                  <td className="px-4 py-3 text-sm font-semibold text-text-primary">{i.preplan_id || '—'}</td>
                  <td className="px-4 py-3 text-sm text-text-secondary">
...
function ArsonTab() {
  return (
    <div className="space-y-4">
      <div className="bg-orange-900/10 border border-orange-500/20 chamfer-8 p-4">
        <div className="text-sm font-semibold text-orange-400 mb-1">Restricted Module</div>
