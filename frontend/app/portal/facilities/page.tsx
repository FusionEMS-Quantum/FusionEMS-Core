...
export default function FacilitiesPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg-void">
      <div className="border-b border-border-subtle bg-bg-panel/50 px-5 py-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-black text-text-primary uppercase tracking-widest">Facility Command Center</div>
        ...
        <div className="grid grid-cols-4 gap-3 mt-4">
          {[
            ...
            <div key={metric.label} className="bg-bg-panel border border-border-subtle chamfer-8 px-4 py-3">
              <div className={`text-2xl font-black ${metric.color}`}>{metric.value}</div>
              <div className="text-micro text-text-muted mt-0.5">{metric.label}</div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 p-5 grid grid-cols-12 gap-4">
        <div className="col-span-4 space-y-4">
          <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
            <div className="text-micro uppercase tracking-widest text-text-muted">Facility Network Directory</div>

            <div className="grid grid-cols-2 gap-2">
              <input
                value={search}
                onChange={(event) => setSearch(event.target.value)}
                className="col-span-2 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary focus:outline-none focus:border-brand-orange/60"
                    ...
                    <button
                      ...
                      className={`w-full text-left border chamfer-8 px-3 py-2 transition-colors ${
                        selected
                          ...
                          : 'border-border-subtle bg-bg-void hover:border-brand-orange/30'
                      }`}
                    >
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-sm font-semibold text-text-primary truncate">{facility.name}</span>
                      ...
                      <div className="mt-1 text-micro text-text-muted flex items-center gap-2 flex-wrap">
          ...
          <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
            <div className="text-micro uppercase tracking-widest text-text-muted">Add Facility</div>
            <div className="grid grid-cols-2 gap-2">
              <input
                value={newFacility.name}
                onChange={(event) => setNewFacility((prev) => ({ ...prev, name: event.target.value }))}
                className="col-span-2 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                placeholder="Facility name"
              />
              <select
                value={newFacility.facility_type}
                onChange={(event) => setNewFacility((prev) => ({ ...prev, facility_type: event.target.value }))}
                className="bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
              >
        ...
        <div className="col-span-8 space-y-4">
          {!selectedFacility ? (
            <div className="bg-bg-panel border border-border-subtle chamfer-8">
              <QuantumEmptyState
            ...
            <>
              <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-lg font-bold text-text-primary">{selectedFacility.name}</div>
                    <div className="text-micro text-text-muted mt-1 flex items-center gap-2 flex-wrap">
              ...
              {selectedFacilityId && detailLoadingByFacility[selectedFacilityId] ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Relationship State</div>
                    <select
                      ...
                      className="w-full bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                    >
                      {RELATIONSHIP_STATES.map((state) => <option key={state} value={state}>{state}</option>)}
                    </select>

                    <div className="text-micro uppercase tracking-widest text-text-muted pt-2">Destination Preference Notes</div>
                    <textarea
                      ...
                      className="w-full h-28 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                      placeholder="Document destination workflow, handoff constraints, and transport preferences."
                    />

                    <div className="text-micro uppercase tracking-widest text-text-muted pt-2">Service Notes</div>
                    <textarea
                      value={facilityEdit?.service_notes || ''}
                      onChange={(event) => setFacilityEdit((prev) => prev ? { ...prev, service_notes: event.target.value } : prev)}
                      className="w-full h-24 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Operational Snapshot</div>
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        { label: 'Active Friction Flags', value: selectedFriction.filter((flag) => flag.is_active).length, color: 'text-red-400' },
                        { label: 'Active Warnings', value: selectedWarnings.filter((warning) => warning.is_active).length, color: 'text-orange-300' },
                        ...
                        <div key={item.label} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-3">
                          <div className={`text-2xl font-black ${item.color}`}>{item.value}</div>
                          <div className="text-micro text-text-muted mt-0.5">{item.label}</div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Service Profiles</div>
                    {selectedServices.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
                        {selectedServices.map((service) => (
                          <div key={service.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between">
                              <div className="text-sm font-semibold text-text-primary">{service.service_line}</div>
                            ...
                            {service.capability_notes && (
                              <div className="text-micro text-text-muted mt-1">{service.capability_notes}</div>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Add Service Profile</div>
                    <input
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Facility Contacts</div>
                    {selectedContacts.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
                        {selectedContacts.map((contact) => (
                          <div key={contact.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between">
                            ...
                            {contact.notes && <div className="text-micro text-text-muted mt-1">{contact.notes}</div>}
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Add Contact</div>
                    <input
                    ...
                    <textarea
                      ...
                      className="w-full h-20 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Friction Board</div>
                    {selectedFriction.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
                        {selectedFriction.map((flag) => (
                          <div key={flag.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between gap-2">
                              <div className="text-sm font-semibold text-text-primary">{flag.title}</div>
                              <span className={`text-[10px] font-bold px-2 py-0.5 chamfer-4 border ${
                                flag.is_active
                            ...
                            <div className="text-micro text-text-muted mt-1">{flag.category} • {new Date(flag.created_at).toLocaleString()}</div>
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Raise Friction Flag</div>
                    <select
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Warning Flags</div>
                    {selectedWarnings.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
                        {selectedWarnings.map((warning) => (
                          <div key={warning.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between">
                              <div className="text-sm font-semibold text-text-primary">{warning.title}</div>
                              <span className={`text-[10px] font-bold px-2 py-0.5 chamfer-4 border ${severityStyle(warning.severity)}`}>
                                {warning.severity}
                              </span>
                            </div>
                            <div className="text-micro text-text-muted mt-1">{warning.flag_type} • {new Date(warning.created_at).toLocaleString()}</div>
                            <div className="text-sm text-text-secondary mt-1">{warning.description}</div>
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Create Warning</div>
                    <select
                ...
                <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                  <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Relationship Timeline</div>
                  {selectedTimeline.length === 0 ? (
                    ...
                    <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
                      {selectedTimeline.map((event) => (
                        <div key={event.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-sm font-semibold text-text-primary">{event.title}</span>
                            <span className="text-[10px] text-text-muted font-mono">{new Date(event.created_at).toLocaleString()}</span>
                          </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Relationship Notes</div>
                    {selectedNotes.length === 0 ? (
                      <QuantumEmptyState title="No notes" description="Capture handoff, operational, and billing relationship context." />
                    ) : (
                      <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
                        {selectedNotes.map((note) => (
                          <div key={note.id} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between">
                              <span className="text-xs font-bold text-brand-orange uppercase tracking-widest">{note.note_type}</span>
                              <span className="text-[10px] text-text-muted font-mono">{new Date(note.created_at).toLocaleString()}</span>
                            </div>
                            <div className="text-sm text-text-secondary mt-1 whitespace-pre-wrap">{note.content}</div>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4 space-y-3">
                    <div className="text-micro uppercase tracking-widest text-text-muted">Add Note</div>
                    <input
                    ...
                    <textarea
                      ...
                      className="w-full h-32 bg-bg-void border border-border-subtle chamfer-4 px-3 py-2 text-sm text-text-primary"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">AI Relationship Issues</div>
                    {facilityIssues.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
                        {facilityIssues.map((issue, index) => (
                          <div key={`${issue.issue}-${index}`} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between gap-2">
                              <span className="text-sm font-semibold text-text-primary">{issue.issue}</span>
                              <span className={`text-[10px] font-bold px-2 py-0.5 chamfer-4 border ${severityStyle(issue.severity)}`}>
                                {issue.severity}
                              </span>
                            </div>
                            <div className="text-micro text-text-muted mt-1">{issue.source} • confidence: {issue.confidence} • review: {issue.human_review}</div>
                  ...
                  <div className="bg-bg-panel border border-border-subtle chamfer-8 p-4">
                    <div className="text-micro uppercase tracking-widest text-text-muted mb-3">Command Top Actions</div>
                    {topActions.length === 0 ? (
                      ...
                      <div className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
                        {topActions.map((action, index) => (
                          <div key={`${action.title}-${index}`} className="bg-bg-void border border-border-subtle chamfer-8 px-3 py-2">
                            <div className="flex items-center justify-between">
                              <div className="text-sm font-semibold text-text-primary">#{action.priority} {action.title}</div>
