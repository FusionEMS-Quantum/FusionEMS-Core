...
function SeverityBadge({ severity }: { severity: string }) {
  const cls =
    ...
    severity === 'HIGH' ? 'bg-[rgba(255,77,0,0.3)] border-orange-500/40 text-[#FF9A66]' :
...
function KPIBar({ kpis }: { kpis: BillingKPI[] }) {
  return (
    <div className="grid grid-cols-4 gap-3 mb-5">
      {kpis.map(kpi => (
        <div key={kpi.label} className="bg-[#0A0A0B] border border-border-subtle chamfer-8 px-4 py-3">
          <div className={`text-2xl font-black ${kpi.color || 'text-zinc-100'}`}>
            {kpi.value}
            {kpi.trend != null && <TrendBadge trend={kpi.trend} />}
          </div>
          <div className="text-micro text-zinc-500 mt-0.5">{kpi.label}</div>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="bg-[#0A0A0B] border border-border-subtle chamfer-8 p-4">
        ...
        {data.map((pt, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-3">
              <span className="text-micro font-mono text-zinc-500 w-16 text-right flex-shrink-0">{pt.period || '—'}</span>
              <div className="flex-1 space-y-0.5">
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 bg-zinc-950/[0.04] chamfer-4 overflow-hidden">
                    <div className="h-full bg-blue-500/60 chamfer-4" style={{ width: `${(billed / maxBilled) * 100}%` }} />
                  </div>
                  <span className="text-micro text-zinc-500 w-20 text-right">{fmt$(billed)}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-2 bg-zinc-950/[0.04] chamfer-4 overflow-hidden">
                    <div className="h-full bg-green-500/60 chamfer-4" style={{ width: `${(collected / maxBilled) * 100}%` }} />
                  </div>
                  <span className="text-micro text-zinc-500 w-20 text-right">{fmtPct(collectionPct)} coll.</span>
                </div>
              </div>
            </div>
          );
      ...
      <div className="flex items-center gap-4 mt-3">
        <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-blue-500/60 chamfer-4" /><span className="text-micro text-zinc-500">Billed</span></div>
        <div className="flex items-center gap-1.5"><div className="w-3 h-2 bg-green-500/60 chamfer-4" /><span className="text-micro text-zinc-500">Collected</span></div>
      </div>
    </div>
  );
}
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-border-subtle hover:bg-zinc-950/[0.02]">
              <td className="px-4 py-3 text-sm font-semibold text-zinc-100">{row.payer_name || '—'}</td>
              <td className="px-4 py-3 text-sm text-zinc-400">{row.volume ?? '—'}</td>
              ...
              <td className="px-4 py-3 text-sm text-zinc-400">{row.avg_days_to_pay != null ? `${row.avg_days_to_pay}d` : '—'}</td>
              <td className="px-4 py-3 text-sm font-mono text-zinc-400">{row.total_paid != null ? fmt$(row.total_paid) : '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function DenialHeatmap({ denials }: { denials: DenialEntry[] }) {
  ...
  return (
    <div className="bg-[#0A0A0B] border border-border-subtle chamfer-8 p-4">
      <div className="text-micro uppercase tracking-widest text-zinc-500 mb-4">Top Denial Reasons</div>
      <div className="space-y-2">
        {topDenials.map((d, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-micro font-mono text-zinc-500 w-14 text-right flex-shrink-0">{d.code || '—'}</span>
          ...
          <table className="w-full text-left">
            ...
            <tbody>
              {(data.categories || []).map((cat, i) => (
                <tr key={i} className="border-b border-border-subtle hover:bg-zinc-950/[0.02]">
                  <td className="px-4 py-3 text-sm font-semibold text-zinc-100">{cat.label}</td>
                  <td className="px-4 py-3 text-sm text-zinc-400">{cat.count}</td>
    ...
    <div className="bg-[#0A0A0B] border border-border-subtle chamfer-8 overflow-hidden">
      <table className="w-full text-left">
        ...
        <tbody>
          {anomalies.map((a, i) => (
            <tr key={i} className="border-b border-border-subtle hover:bg-zinc-950/[0.02]">
              <td className="px-4 py-3 text-xs font-mono text-zinc-500">{a.claim_id?.slice(0, 12) || '—'}</td>
              <td className="px-4 py-3 text-sm text-zinc-400">{a.type || '—'}</td>
              <td className="px-4 py-3">
                {a.score != null ? (
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-zinc-950/10  overflow-hidden">
                      <div className={`h-full  ${a.score >= 80 ? 'bg-red-500' : a.score >= 50 ? 'bg-[#FF4D00]' : 'bg-yellow-500'}`}
                           style={{ width: `${a.score}%` }} />
                    </div>
              ...
              <td className="px-4 py-3 text-sm text-zinc-500 max-w-xs truncate">{a.detail || '—'}</td>
    ...
    <div className="flex flex-col bg-black min-h-screen">
      ...
      <div className="flex-shrink-0 border-b border-border-subtle bg-[#0A0A0B]/50 px-5 py-4">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-black text-zinc-100 uppercase tracking-widest">Billing Command Center</div>
            <div className="text-micro text-zinc-500">KPIs · Payer performance · Denial intelligence · Revenue leakage · Fraud detection</div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="space-y-3">
              {alerts.map((alert, i) => (
                <div key={alert.id || i} className={`chamfer-8 border px-4 py-3 flex items-start gap-3 ${
                  ...
                  alert.severity === 'HIGH' ? 'bg-[rgba(255,77,0,0.2)] border-orange-500/30' :
                  alert.severity === 'MEDIUM' ? 'bg-yellow-900/20 border-yellow-500/30' :
                  'bg-[#0A0A0B] border-border-subtle'
                }`}>
                  <SeverityBadge severity={alert.severity || 'INFO'} />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-zinc-100">{alert.message || '—'}</div>
                    {alert.created_at && (
                      <div className="text-micro text-zinc-500 mt-0.5">{new Date(alert.created_at).toLocaleString()}</div>
                    )}
                  </div>
                </div>
              ))}
