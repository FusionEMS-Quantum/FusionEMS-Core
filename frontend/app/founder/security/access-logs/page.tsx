...
const LEVEL_COLOR: Record<string, string> = {
  auth_event: 'var(--color-brand-orange)',
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-micro font-mono text-text-muted w-36 flex-shrink-0">{formatTs(log.created_at)}</span>
        <span className="text-xs font-semibold w-28 flex-shrink-0" style={{ color: c }}>
          {log.domain.replace(/_/g, ' ')}
        </span>
        <span className="text-xs text-text-primary flex-1 truncate">{log.action}</span>
        <span className="text-micro text-text-muted font-mono truncate w-32 flex-shrink-0">
          {log.actor_email ?? log.actor_user_id.slice(0, 8) + '…'}
        </span>
        <span className="text-micro text-text-muted font-mono">{log.ip_address ?? '—'}</span>
      ...
      {expanded && (
        <div className="px-4 pb-4 bg-white/[0.02]">
          <div className="grid grid-cols-2 gap-2 mb-2 text-micro">
            <div><span className="text-text-muted">Entity: </span><span className="font-mono text-text-secondary">{log.entity_name} / {log.entity_id?.slice(0, 8)}…</span></div>
            <div><span className="text-text-muted">Correlation: </span><span className="font-mono text-text-secondary">{log.correlation_id ?? '—'}</span></div>
            <div><span className="text-text-muted">Actor ID: </span><span className="font-mono text-text-secondary">{log.actor_user_id}</span></div>
            <div><span className="text-text-muted">Tenant: </span><span className="font-mono text-text-secondary">{log.tenant_id?.slice(0, 8)}…</span></div>
          </div>
          {Object.keys(log.metadata_json ?? {}).length > 0 && (
            <pre className="bg-bg-page text-micro font-mono text-text-secondary p-2 rounded overflow-auto max-h-40">
...
export default function AccessLogsPage() {
  ...
  return (
    <div className="p-5 min-h-screen">
      ...
      <div className="hud-rail pb-3 mb-6 flex items-end justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={exportCsv} disabled={logs.length === 0}
            className="px-3 py-1.5 text-micro font-semibold uppercase tracking-widest border border-border-DEFAULT text-text-secondary hover:text-text-primary hover:border-orange transition-colors disabled:opacity-40">
            Export CSV
          </button>
      ...
      <div className="flex gap-1 mb-4 overflow-x-auto pb-1">
        {DOMAINS.map(d => (
          <button key={d} onClick={() => setDomain(d)}
            className={`px-3 py-1 text-micro font-semibold uppercase tracking-widest whitespace-nowrap transition-colors border ${domain === d ? 'border-orange text-orange bg-orange/10' : 'border-border-DEFAULT text-text-muted hover:text-text-primary'}`}>
      ...
      {/* Search bar */}
      <form onSubmit={handleSearch} className="flex flex-wrap gap-2 mb-4 bg-bg-panel border border-border-DEFAULT p-3"
        style={{ clipPath: 'polygon(0 0, calc(100% - 8px) 0, 100% 8px, 100% 100%, 0 100%)' }}>
        <input value={action} onChange={e => setAction(e.target.value)} placeholder="Action keyword…"
          className="flex-1 min-w-0 bg-bg-page border border-border-DEFAULT px-3 py-1.5 text-xs text-text-primary placeholder:text-text-muted focus:outline-none focus:border-orange" />
        <input value={actor} onChange={e => setActor(e.target.value)} placeholder="Actor user ID…"
          className="w-48 bg-bg-page border border-border-DEFAULT px-3 py-1.5 text-xs text-text-primary placeholder:text-text-muted focus:outline-none focus:border-orange" />
        <input type="datetime-local" value={fromDt} onChange={e => setFromDt(e.target.value)}
          className="w-48 bg-bg-page border border-border-DEFAULT px-3 py-1.5 text-xs text-text-primary focus:outline-none focus:border-orange" />
        <input type="datetime-local" value={toDt} onChange={e => setToDt(e.target.value)}
          className="w-48 bg-bg-page border border-border-DEFAULT px-3 py-1.5 text-xs text-text-primary focus:outline-none focus:border-orange" />
        <button type="submit"
          className="px-4 py-1.5 text-micro font-semibold uppercase tracking-widest bg-orange text-bg-page hover:bg-orange-bright transition-colors">
          Search
        </button>
      </form>
      ...
      <div className="bg-bg-panel border border-border-DEFAULT"
        ...
        <div className="flex items-center gap-3 px-4 py-2 border-b border-white/10 bg-white/[0.02]">
          ...
          <span className="text-micro uppercase tracking-widest text-text-muted w-36 flex-shrink-0">Time</span>
          <span className="text-micro uppercase tracking-widest text-text-muted w-28 flex-shrink-0">Domain</span>
          <span className="text-micro uppercase tracking-widest text-text-muted flex-1">Action</span>
          <span className="text-micro uppercase tracking-widest text-text-muted w-32 flex-shrink-0">Actor</span>
          <span className="text-micro uppercase tracking-widest text-text-muted">IP</span>
        ...
        {loading && !logs.length && (
          <div className="px-4 py-12 text-center text-text-muted text-sm">Loading audit logs…</div>
        )}

        {!loading && !error && logs.length === 0 && (
          <div className="px-4 py-12 text-center text-text-muted text-sm">No logs match the current filters.</div>
        )}
        ...
        <div className="flex items-center justify-between mt-4 text-micro text-text-muted">
          ...
          <div className="flex gap-2">
            <button disabled={offset === 0} onClick={() => { setOffset(o => Math.max(0, o - LIMIT)); fetchLogs(); }}
              className="px-3 py-1 border border-border-DEFAULT hover:border-orange disabled:opacity-40 transition-colors text-text-secondary hover:text-text-primary">
              ← Prev
            </button>
