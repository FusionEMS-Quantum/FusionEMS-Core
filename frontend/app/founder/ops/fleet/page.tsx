...
export default function FleetTelemetryPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <Panel>
        <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mb-3">Unit Health States</div>
        <div className="flex flex-wrap gap-2">
          {Object.entries(TELEMETRY_STATE_COLORS).map(([state, colors]) => (
            <span key={state} className="px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider"
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            ...
            <div key={item.label} className="rounded-xl p-4 border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.03)] text-center">
              <div className="text-3xl font-black" style={{ color: item.color }}>{item.value}</div>
              <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mt-1">{item.label}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {criticalAlerts.length > 0 && (
        <div className="p-4 rounded-xl border border-red-500/40 bg-[rgba(229,57,53,0.08)]">
      ...
      <div className="flex gap-2">
        {(['readiness', 'alerts', 'ingest'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider ${activeTab === tab ? 'bg-green-700 text-white' : 'bg-[rgba(255,255,255,0.06)] text-[rgba(255,255,255,0.5)]'}`}>
            ...
            <Panel key={unit.unit_id}>
              <div className="flex items-center gap-4">
                <div className="flex-shrink-0 w-32">
                  <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.4)]">Unit</div>
                  <div className="text-sm font-bold text-white truncate">{unit.unit_id.slice(0, 12)}</div>
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] uppercase tracking-wider" style={{ color: READINESS_COLOR(unit.readiness_score) }}>
      ...
      {activeTab === 'alerts' && (
        <div className="space-y-2">
          ...
          {activeAlerts.map(alert => {
            ...
            return (
              <div key={alert.id} className="rounded-xl border p-4 flex items-center justify-between"
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[10px] text-[rgba(255,255,255,0.4)]">Unit: {d.unit_id?.slice(0, 12)}</span>
                    {d.fault_type && <span className="text-[10px] text-[rgba(255,255,255,0.35)]">{d.fault_type}</span>}
                  </div>
                  <div className="text-sm text-white">{d.message}</div>
                </div>
                {!d.acknowledged && (
                  <button onClick={() => ackAlert(alert.id, 1)}
                    className="px-3 py-1.5 bg-[rgba(255,255,255,0.08)] border border-[rgba(255,255,255,0.15)] text-[11px] font-bold text-white rounded-lg hover:bg-[rgba(255,255,255,0.12)] ml-4">
                    ACK
                  </button>
                )}
              </div>
        ...
        <Panel>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <input value={ingestForm.unit_id} onChange={e => setIngestForm(p => ({ ...p, unit_id: e.target.value }))}
              placeholder="Unit ID *"
              className="h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-green-400 placeholder-[rgba(255,255,255,0.25)]" />
            {[
              ...
              <div key={field.key}>
                <input
                  ...
                  className="w-full h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-green-400 placeholder-[rgba(255,255,255,0.2)]" />
              </div>
            ))}
          </div>
          <button onClick={ingestTelemetry} disabled={ingesting || !ingestForm.unit_id}
            className="px-6 py-2 bg-green-700 text-white text-sm font-bold rounded-lg hover:bg-green-600 disabled:opacity-40 transition-colors">
            {ingesting ? 'Processing…' : '📡 Ingest Telemetry'}
          </button>

          {ingestResult && (
            <div className="mt-4 p-4 rounded-lg bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)]">
              <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.4)] mb-2">Ingest Result</div>
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <div className="text-[10px] text-[rgba(255,255,255,0.3)]">Event ID</div>
                  <div className="text-[11px] text-white font-mono">{String(ingestResult.telemetry_event_id ?? '—').slice(0, 16)}</div>
                </div>
                <div>
                  <div className="text-[10px] text-[rgba(255,255,255,0.3)]">Faults Detected</div>
                ...
                <div>
                  <div className="text-[10px] text-[rgba(255,255,255,0.3)]">Alerts Created</div>
