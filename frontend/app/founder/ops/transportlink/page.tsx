...
export default function TransportLinkPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg">{toast}</div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="px-3 py-1.5 bg-[rgba(255,193,7,0.1)] border border-[rgba(255,193,7,0.3)] rounded-lg">
            <span className="text-yellow-400 text-sm font-bold">{pending.length}</span>
            <span className="text-[10px] text-[rgba(255,255,255,0.4)] ml-1">PENDING REVIEW</span>
          </div>
          <div className="px-3 py-1.5 bg-[rgba(255,107,26,0.1)] border border-[rgba(255,107,26,0.3)] rounded-lg">
            <span className="text-orange-400 text-sm font-bold">{injected.length}</span>
            <span className="text-[10px] text-[rgba(255,255,255,0.4)] ml-1">IN CAD</span>
          </div>
        </div>
      </div>
      ...
      {/* ── Request Flow ── */}
      <Panel>
        <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mb-3">Request State Flow</div>
        <div className="flex flex-wrap gap-1.5">
          {['REQUEST_CREATED','REQUEST_VALIDATED','REQUEST_ACCEPTED','DISPATCH_INJECTED','ASSIGNMENT_PENDING','CLOSED'].map((s, i, arr) => (
            <span key={s} className="flex items-center gap-1">
              <StateBadge state={s} />
              {i < arr.length - 1 && <span className="text-[rgba(255,255,255,0.2)] text-xs">→</span>}
            </span>
          ))}
          <span className="text-[rgba(255,255,255,0.2)] text-xs mx-1">or</span>
          <StateBadge state="REQUEST_REJECTED" />
        </div>
        <div className="mt-3 text-[11px] text-[rgba(255,255,255,0.45)]">
      ...
      <div className="flex gap-2">
        {(['requests', 'new'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg text-[11px] font-bold uppercase tracking-wider ${activeTab === tab ? 'bg-purple-700 text-white' : 'bg-[rgba(255,255,255,0.06)] text-[rgba(255,255,255,0.5)]'}`}>
      ...
      {activeTab === 'requests' && (
        <div className="space-y-2">
          {loading && <div className="text-sm text-[rgba(255,255,255,0.4)] p-4">Loading requests…</div>}
          {!loading && requests.length === 0 && (
            <div className="text-center py-10 rounded-xl border border-[rgba(255,255,255,0.08)]">
              <div className="text-3xl mb-2">🏥</div>
              <div className="text-sm text-[rgba(255,255,255,0.45)]">No facility transport requests. Create one using the New Request tab.</div>
            </div>
          )}
          {requests.map(req => {
            const d = req.data ?? {};
            ...
            return (
              <div key={req.id} className="rounded-xl border border-[rgba(255,255,255,0.08)] overflow-hidden">
                <button onClick={() => setSelected(isSelected ? null : req)}
                  ...
                  <div className="flex-shrink-0">
                    ...
                    <div className="text-[10px] text-[rgba(255,255,255,0.4)] mt-1">{d.service_level} · {d.priority}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-white">{d.origin_facility} → {d.destination_facility}</div>
                    <div className="text-[11px] text-[rgba(255,255,255,0.5)]">{d.chief_complaint || 'No complaint noted'}</div>
                    <div className="text-[11px] text-[rgba(255,255,255,0.35)]">{d.origin_address}</div>
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <div className="text-[10px] text-[rgba(255,255,255,0.4)]">{d.contact_name}</div>
                    <div className="text-[10px] text-[rgba(255,255,255,0.3)]">{d.contact_phone}</div>
                  </div>
                  <span className="text-[rgba(255,255,255,0.2)] text-sm ml-2">{isSelected ? '▲' : '▼'}</span>
                </button>
                ...
                {isSelected && (
                  <div className="px-4 pb-4 border-t border-[rgba(255,255,255,0.06)] space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Request ID</div>
                        <div className="text-[11px] text-white font-mono">{req.id.slice(0, 16)}</div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Origin</div>
                        <div className="text-[11px] text-white">{d.origin_address}</div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Destination</div>
                        <div className="text-[11px] text-white">{d.destination_address || d.destination_facility}</div>
                      </div>
                      <div>
                        <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Mission ID</div>
                        <div className="text-[11px] text-white">{d.mission_id ? d.mission_id.slice(0, 12) : 'Not yet injected'}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      {d.state === 'REQUEST_CREATED' && (
                        <button onClick={() => validate(req.id)}
                          className="px-4 py-1.5 bg-[rgba(41,182,246,0.15)] border border-[rgba(41,182,246,0.3)] text-blue-400 text-[11px] font-bold rounded-lg hover:bg-[rgba(41,182,246,0.2)]">
                          ✓ Validate Request
                        </button>
                      )}
                      {['REQUEST_CREATED', 'REQUEST_VALIDATED'].includes(d.state) && (
                        <button onClick={() => injectToCad(req.id)}
                          className="px-4 py-1.5 bg-[rgba(255,107,26,0.15)] border border-[rgba(255,107,26,0.3)] text-orange-400 text-[11px] font-bold rounded-lg hover:bg-[rgba(255,107,26,0.2)]">
                          📡 Inject to CAD
                        </button>
                      )}
                      {d.mission_id && (
                        <Link href="/founder/ops/cad" className="px-4 py-1.5 bg-[rgba(76,175,80,0.1)] border border-[rgba(76,175,80,0.3)] text-green-400 text-[11px] font-bold rounded-lg hover:bg-[rgba(76,175,80,0.15)]">
                          View in CAD →
                        </Link>
                      )}
                    </div>
      ...
      {activeTab === 'new' && (
        <Panel>
          <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mb-4">
            New Interfacility Transport Request
          </div>
          <div className="text-[11px] text-[rgba(255,255,255,0.5)] mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
            <input value={newReq.origin_facility} onChange={e => setNewReq(p => ({ ...p, origin_facility: e.target.value }))}
              ...
              className="h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-purple-400 placeholder-[rgba(255,255,255,0.25)]" />
            ...
            <select value={newReq.service_level} onChange={e => setNewReq(p => ({ ...p, service_level: e.target.value }))}
              className="h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-purple-400">
          ...
          <button onClick={createRequest} disabled={creating || !newReq.origin_address || !newReq.origin_facility}
            className="px-6 py-2 bg-purple-700 text-white text-sm font-bold rounded-lg hover:bg-purple-600 disabled:opacity-40 transition-colors">
