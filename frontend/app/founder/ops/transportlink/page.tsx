...
const REQUEST_STATE_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  DISPATCH_INJECTED:  { bg: 'rgba(255,106,0,0.15)', text: '#ff6b1a' },
...
export default function TransportLinkPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-2">
            <Link
              href="/founder/ops/transportlink/transport-billing-payment"
                className="px-3 py-1.5 bg-[rgba(255,106,0,0.12)] border border-orange-400/[0.35] text-[var(--q-orange)] text-body font-bold chamfer-8 hover:bg-[var(--q-orange)]/[0.2]"
            >
          ...
          <div className="flex items-center gap-2">
            <Link
              ...
              className="px-3 py-1.5 bg-[var(--color-status-info)]/[0.12] border border-blue-400/[0.35] text-[var(--color-status-info)] text-body font-bold chamfer-8 hover:bg-[var(--color-status-info)]/[0.2]"
            >
              Hospital Scheduling
            </Link>
            <Link
              href="/founder/ops/transportlink/assisted-living"
              className="px-3 py-1.5 bg-[var(--color-status-active)]/[0.12] border border-[var(--color-status-active)]/[0.35] text-[var(--color-status-active)] text-body font-bold chamfer-8 hover:bg-[var(--color-status-active)]/[0.2]"
            >
              Assisted Living Scheduling
            </Link>
            <div className="px-3 py-1.5 bg-amber-400/[0.1] border border-amber-400/[0.3] chamfer-8">
              <span className="text-[var(--q-yellow)] text-sm font-bold">{pending.length}</span>
              <span className="text-micro text-[var(--color-text-muted)] ml-1">PENDING REVIEW</span>
            </div>
            <div className="px-3 py-1.5 bg-brand-orange/[0.1] border border-brand-orange/[0.3] chamfer-8">
                <span className="text-[var(--q-orange)] text-sm font-bold">{injected.length}</span>
              <span className="text-micro text-[var(--color-text-muted)] ml-1">IN CAD</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* ── Request Flow ── */}
      <Panel>
        <div className="text-micro uppercase tracking-widest text-[var(--color-text-muted)] mb-3">Request State Flow</div>
        <div className="flex flex-wrap gap-1.5">
          {['REQUEST_CREATED','REQUEST_VALIDATED','REQUEST_ACCEPTED','DISPATCH_INJECTED','ASSIGNMENT_PENDING','CLOSED'].map((s, i, arr) => (
            <span key={s} className="flex items-center gap-1">
              <StateBadge state={s} />
              {i < arr.length - 1 && <span className="text-[var(--color-text-muted)] text-xs">→</span>}
            </span>
          ))}
      ...
      <div className="flex gap-2">
        {(['requests', 'new'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 chamfer-8 text-body font-bold uppercase tracking-wider ${activeTab === tab ? 'bg-purple-700 text-white' : 'bg-[var(--color-bg-base)]/[0.06] text-[var(--color-text-secondary)]'}`}>
      ...
      {activeTab === 'requests' && (
        <div className="space-y-2">
          {loading && <div className="text-sm text-[var(--color-text-muted)] p-4">Loading requests…</div>}
          {!loading && requests.length === 0 && (
            <div className="text-center py-10 chamfer-4-xl border border-white/[0.08]">
              <div className="text-3xl mb-2">🏥</div>
              <div className="text-sm text-[var(--color-text-muted)]">No facility transport requests. Create one using the New Request tab.</div>
            </div>
          )}
          {requests.map(req => {
            const d = req.data ?? {};
              ...
              <div key={req.id} className="chamfer-4-xl border border-white/[0.08] overflow-hidden">
                <button onClick={() => setSelected(isSelected ? null : req)}
                  ...
                  <div className="flex-shrink-0">
                    ...
                    <div className="text-micro text-[var(--color-text-muted)] mt-1">{d.service_level} · {d.priority}</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-white">{d.origin_facility} → {d.destination_facility}</div>
                    <div className="text-body text-[var(--color-text-secondary)]">{d.chief_complaint || 'No complaint noted'}</div>
                    <div className="text-body text-[var(--color-text-muted)]">{d.origin_address}</div>
                  </div>
                  <div className="flex-shrink-0 text-right">
                    <div className="text-micro text-[var(--color-text-muted)]">{d.contact_name}</div>
                    <div className="text-micro text-[var(--color-text-muted)]">{d.contact_phone}</div>
                  </div>
                  <span className="text-[var(--color-text-muted)] text-sm ml-2">{isSelected ? '▲' : '▼'}</span>
                </button>
                ...
                {isSelected && (
                  <div className="px-4 pb-4 border-t border-white/[0.06] space-y-4">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                      <div>
                        <div className="text-micro uppercase tracking-wider text-[var(--color-text-muted)]">Request ID</div>
                        <div className="text-body text-white font-mono">{req.id.slice(0, 16)}</div>
                      </div>
                      <div>
                        <div className="text-micro uppercase tracking-wider text-[var(--color-text-muted)]">Origin</div>
                    ...
                    <div className="flex gap-2">
                      {d.state === 'REQUEST_CREATED' && (
                        <button onClick={() => validate(req.id)}
                          className="px-4 py-1.5 bg-[var(--color-status-info)]/[0.15] border border-blue-400/[0.3] text-[var(--color-status-info)] text-body font-bold chamfer-8 hover:bg-[var(--color-status-info)]/[0.2]">
                          ✓ Validate Request
                        </button>
                      )}
                      {['REQUEST_CREATED', 'REQUEST_VALIDATED'].includes(d.state) && (
                        <button onClick={() => injectToCad(req.id)}
                            className="px-4 py-1.5 bg-brand-orange/[0.15] border border-brand-orange/[0.3] text-[var(--q-orange)] text-body font-bold chamfer-8 hover:bg-brand-orange/[0.2]">
                          📡 Inject to CAD
                        </button>
                      )}
                      {d.mission_id && (
                        <Link href="/founder/ops/cad" className="px-4 py-1.5 bg-[var(--color-status-active)]/[0.1] border border-[var(--color-status-active)]/30 text-[var(--color-status-active)] text-body font-bold chamfer-8 hover:bg-[var(--color-status-active)]/[0.15]">
                          View in CAD →
                        </Link>
                      )}
                    </div>
      ...
      {activeTab === 'new' && (
        <Panel>
          <div className="text-micro uppercase tracking-widest text-[var(--color-text-muted)] mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
            <input value={newReq.origin_facility} onChange={e => setNewReq(p => ({ ...p, origin_facility: e.target.value }))}
              ...
              className="h-9 bg-[var(--color-bg-base)]/[0.06] border border-white/[0.12] px-3 text-sm text-white chamfer-8 focus:outline-none focus:border-purple-400 placeholder-text-[var(--color-text-muted)]" />
            ...
            <select value={newReq.service_level} onChange={e => setNewReq(p => ({ ...p, service_level: e.target.value }))}
              className="h-9 bg-[var(--color-bg-base)]/[0.06] border border-white/[0.12] px-3 text-sm text-white chamfer-8 focus:outline-none focus:border-purple-400">
