...
export default function CrewLinkPagingPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-lg">{toast}</div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="px-3 py-2 rounded-xl border border-[rgba(255,107,26,0.4)] bg-[rgba(255,107,26,0.1)]">
          <div className="text-[10px] uppercase tracking-wider text-orange-400">Boundary Enforced</div>
          <div className="text-[11px] text-white">Operations only · No billing content</div>
        </div>
      </div>

      {/* ── Paging Flow ── */}
      <Panel>
        <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mb-3">Paging State Flow</div>
        <div className="flex flex-wrap gap-1.5">
          {PAGING_FLOW.map((s, i) => (
            <span key={s} className="flex items-center gap-1">
              <StateBadge state={s} />
              {i < PAGING_FLOW.length - 1 && <span className="text-[rgba(255,255,255,0.2)] text-xs">→</span>}
            </span>
          ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={item.label} className="rounded-xl p-4 border text-center" style={{ background: item.bg, borderColor: item.color + '44' }}>
            <div className="text-3xl font-black" style={{ color: item.color }}>{item.value}</div>
            <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.5)] mt-1">{item.label}</div>
          </div>
        ))}
      </div>
      ...
      {/* ── New Alert ── */}
      <Panel>
        <div className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.4)] mb-3">Send CrewLink Page</div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-3">
          <input value={newAlert.mission_id} onChange={e => setNewAlert(p => ({ ...p, mission_id: e.target.value }))}
            placeholder="Mission ID *"
            className="h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-blue-400 placeholder-[rgba(255,255,255,0.25)]" />
          ...
          <select value={newAlert.service_level} onChange={e => setNewAlert(p => ({ ...p, service_level: e.target.value }))}
            className="h-9 bg-[rgba(255,255,255,0.06)] border border-[rgba(255,255,255,0.12)] px-3 text-sm text-white rounded-lg focus:outline-none focus:border-blue-400">
        ...
        <button onClick={createAlert} disabled={creating || !newAlert.mission_id || !newAlert.mission_address}
          className="px-6 py-2 bg-blue-700 text-white text-sm font-bold rounded-lg hover:bg-blue-600 disabled:opacity-40 transition-colors">
      ...
      <div className="flex items-center gap-2">
        {['active','all'].map(f => (
          <button key={f} onClick={() => setFilterState(f)} className={`px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase tracking-wide ${filterState === f ? 'bg-blue-700 text-white' : 'bg-[rgba(255,255,255,0.06)] text-[rgba(255,255,255,0.5)]'}`}>
            {f}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {loading && <div className="text-sm text-[rgba(255,255,255,0.4)] p-4">Loading alerts…</div>}
        {!loading && alerts.length === 0 && (
          <div className="text-center py-10 rounded-xl border border-[rgba(255,255,255,0.08)]">
            <div className="text-3xl mb-2">📟</div>
            <div className="text-sm text-[rgba(255,255,255,0.45)]">No paging alerts. Use the form above to send a page.</div>
          </div>
        )}
        {alerts.map(alert => {
          const d = alert.data ?? {};
          ...
          return (
            <div key={alert.id} className={`rounded-xl border overflow-hidden ${isEscalated ? 'border-red-500/40' : isAccepted ? 'border-green-500/30' : 'border-[rgba(255,255,255,0.08)]'}`}>
              <button onClick={() => setSelected(isSelected ? null : alert)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-[rgba(255,255,255,0.5)]">{d.mission_address}</div>
                  {d.chief_complaint && <div className="text-[11px] text-[rgba(255,255,255,0.4)]">{d.chief_complaint}</div>}
                </div>
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div className="text-right">
                    <div className="text-[10px] text-[rgba(255,255,255,0.4)]">{d.service_level} · {d.priority}</div>
                    <div className="text-[10px] text-[rgba(255,255,255,0.3)]">{d.target_crew_ids?.length ?? 0} crew paged</div>
                  </div>
                  {isEscalated && !isSelected && (
                    <button onClick={e => { e.stopPropagation(); escalate(alert.id); }}
                      className="px-3 py-1 bg-red-700 text-white text-[10px] font-bold rounded-lg hover:bg-red-600">
                      ESCALATE
                    </button>
                  )}
              ...
              {isSelected && (
                <div className="px-4 pb-4 border-t border-[rgba(255,255,255,0.06)] space-y-4">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Alert ID</div>
                      <div className="text-[11px] text-white font-mono">{alert.id.slice(0, 16)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">ACK Deadline</div>
                      <div className="text-[11px] text-white">{d.ack_deadline ? new Date(d.ack_deadline).toLocaleTimeString() : '—'}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Accept Deadline</div>
                      <div className="text-[11px] text-white">{d.accept_deadline ? new Date(d.accept_deadline).toLocaleTimeString() : '—'}</div>
                    </div>
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-[rgba(255,255,255,0.3)]">Accepted By</div>
                      <div className="text-[11px] text-green-400">{d.accepted_by || '—'}</div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {!['ACCEPTED','CLOSED'].includes(d.state) && (
                      <button onClick={() => escalate(alert.id)}
                        className="px-4 py-1.5 bg-[rgba(229,57,53,0.15)] border border-[rgba(229,57,53,0.3)] text-red-400 text-[11px] font-bold rounded-lg hover:bg-[rgba(229,57,53,0.2)]">
                        🔺 Escalate Now
                      </button>
                    )}
                    <Link href={`/founder/ops/crewlink`} className="px-4 py-1.5 bg-[rgba(41,182,246,0.1)] border border-[rgba(41,182,246,0.3)] text-blue-400 text-[11px] font-bold rounded-lg hover:bg-[rgba(41,182,246,0.15)]">
                      View Full Audit →
                    </Link>
                  </div>
                </div>
