...
export default function FounderSpecialtyOpsPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-6">
      <div>
        <div className="text-xs uppercase tracking-[0.2em] text-[var(--q-orange)]/80">Founder Command</div>
        <h1 className="text-2xl font-black text-white">Specialty Ops Command Center</h1>
      </div>

      {error && <div className=" border border-[var(--color-brand-red)]/30 bg-[var(--color-brand-red)]/10 p-3 text-sm text-[var(--color-brand-red)]">{error}</div>}

      {!summary ? (
        <div className=" border border-white/10 bg-[var(--color-bg-base)]/[0.03] p-4 text-sm text-white/70">Loading specialty operations intelligence…</div>
      ) : (
        <>
            ...
            <div className=" border border-white/10 bg-[var(--color-bg-base)]/[0.03] p-4">
              ...
              <div className="space-y-2">
                ...
                {missions.map((mission) => (
                  <div key={mission.id} className=" border border-white/10 bg-[var(--color-bg-base)]/20 p-3">
