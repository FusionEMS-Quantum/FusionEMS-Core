  ...
  return (
    <div className="p-8 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-[var(--color-bg-base)] border border-white/10 p-6">
          ...
          <div className="mb-4">
            ...
            <select 
              ...
              className="w-full bg-[var(--color-bg-panel)] border border-white/20 p-2 text-white"
            >
          ...
          {script ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between text-sm text-white/50 mb-2">
                ...
                <button className="text-[var(--color-status-info)] hover:text-[var(--color-status-info)]">Push to Queue</button>
              </div>
              {script.lines.map((line: any, idx: number) => (
                <div key={idx} className="bg-[var(--color-bg-panel)] p-4 border border-white/5">
                  <div className="flex gap-4">
                    <div className="w-16 flex-shrink-0 text-xs text-[var(--color-status-info)] font-mono">{line.time}</div>
                    <div>
                      <div className="text-xs text-[var(--q-yellow)] font-bold mb-1 uppercase tracking-wider">{line.visual}</div>
