  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <span className="px-2 py-0.5 text-micro font-bold uppercase tracking-widest bg-brand-orange/[0.15] text-[var(--q-orange)] chamfer-4">
          {plan.intent}
        </span>
        {plan.risk_level && (
          <span
      ...
      <p className="text-sm text-[var(--color-text-primary)]">{plan.summary}</p>
      ...
      <div>
        ...
        <div className="space-y-2">
          {plan.actions.map((a, i) => (
            <div key={i} className="bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT chamfer-4 p-3">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-micro font-mono font-bold text-status-info">{a.type}</span>
              </div>
              <pre className="text-micro text-[var(--color-text-secondary)] whitespace-pre-wrap break-all">
        ...
        <div className="space-y-1">
          {plan.acceptance_tests.map((t, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="text-[var(--color-status-active)] text-xs">✓</span>
              <code className="text-xs text-[var(--color-text-primary)] font-mono">{t}</code>
            </div>
          ))}
        </div>
      </div>
      ...
      {plan.notes && (
        <div className="bg-brand-orange/[0.06] border border-brand-orange/[0.15] chamfer-4 p-3">
          ...
          <p className="text-xs text-[var(--color-text-secondary)]">{plan.notes}</p>
        </div>
      )}
    </div>
  );
      ...
      <div className="space-y-1">
        {entries.map(([gate, passed]) => (
          <div key={gate} className="flex items-center gap-3 py-1.5 border-b border-border-subtle">
            <span className={`text-sm ${passed ? 'text-[var(--color-status-active)]' : 'text-red'}`}>
              {passed ? '✓' : '✗'}
            </span>
            <span className="text-xs font-mono text-[var(--color-text-primary)]">{gate}</span>
          </div>
        ))}
      </div>
    </div>
...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      {!isUser && (
        <div className="w-6 h-6 flex-shrink-0  bg-brand-orange/[0.2] border border-brand-orange/[0.4] flex items-center justify-center text-[9px] font-bold text-[var(--q-orange)] mr-2 mt-0.5">
          AI
        </div>
      )}
      <div
        className={`max-w-[80%] px-3 py-2 chamfer-4 text-sm leading-relaxed ${
          isUser
              ? 'bg-orange-ghost border border-brand-orange/[0.2] text-[var(--color-text-primary)]'
            : 'bg-[var(--color-bg-base)]/5 border border-border-DEFAULT text-[var(--color-text-primary)]'
        }`}
      >
        <p className="whitespace-pre-wrap">{msg.content_text}</p>
        {msg.content_json && (
          <div className="mt-2 pt-2 border-t border-border-DEFAULT">
            <span className="text-micro text-[var(--q-orange)] font-semibold uppercase tracking-wider">
              ▣ Action plan attached — see right panel
            </span>
          </div>
        )}
        <div className="mt-1 text-micro text-[var(--color-text-muted)]">
...
export default function FounderCopilotPage() {
  ...
  return (
    <div className="flex h-full bg-[var(--color-bg-base)] text-[var(--color-text-primary)] overflow-hidden" style={{ minHeight: 0 }}>
      {/* Session list */}
      <aside className="w-52 flex-shrink-0 border-r border-border-subtle flex flex-col bg-[var(--color-bg-base)]">
        <div className="flex items-center justify-between px-3 py-3 border-b border-border-subtle">
          ...
          <button
            onClick={newSession}
            className="w-6 h-6 bg-brand-orange/[0.15] border border-brand-orange/[0.3] text-[var(--q-orange)] text-xs chamfer-4 hover:bg-brand-orange/[0.25] transition-colors flex items-center justify-center"
            title="New session"
          >
            +
          </button>
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {sessions.length === 0 && (
            <div className="px-3 py-4 text-xs text-[var(--color-text-muted)] italic">No sessions yet</div>
          )}
          {sessions.map((s) => (
            <button
              key={s.id}
              ...
              className={`w-full text-left px-3 py-2 border-b border-border-subtle transition-colors ${
                activeSession?.id === s.id
                    ? 'bg-orange-ghost text-[var(--color-text-primary)] border-l-2 border-l-[var(--q-orange)]'
                  : 'text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-base)]/[0.04] hover:text-[var(--color-text-primary)]'
              }`}
            >
              <div className="text-xs truncate">{s.title}</div>
              <div className="text-micro text-[var(--color-text-muted)] mt-0.5">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        {/* Chat header */}
        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border-subtle bg-[var(--color-bg-base)] flex-shrink-0">
          <div>
          ...
          {activeRun && (
            <div className="ml-auto flex items-center gap-2">
              <span className="text-micro text-[var(--color-text-muted)]">RUN</span>
              <span
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4">
          {!activeSession && (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-sm text-[var(--color-text-secondary)] mb-2">Founder Copilot</p>
              ...
              <button
                onClick={newSession}
                className="mt-4 px-4 py-2 bg-brand-orange/[0.15] border border-brand-orange/[0.3] text-[var(--q-orange)] text-xs font-semibold chamfer-4 hover:bg-brand-orange/[0.25] transition-colors"
              >
                + New Session
              </button>
            </div>
        ...
        {/* Error */}
        {error && (
          <div className="mx-4 mb-2 px-3 py-2 bg-[var(--color-brand-red)]/10 border border-[var(--color-brand-red)]/20 chamfer-4 text-xs text-[var(--color-brand-red)] flex items-center justify-between">
            <span>{error}</span>
            <button onClick={() => setError(null)} className="ml-2 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]">✕</button>
          </div>
        )}
          ...
          <div className="flex-shrink-0 px-4 pb-4 space-y-2">
            <div className="flex gap-2 flex-wrap">
              <button
                ...
                disabled={!messages.some((m) => m.content_json)}
                  className="px-3 py-1.5 text-xs font-semibold bg-orange-ghost border border-brand-orange/[0.3] text-[var(--q-orange)] chamfer-4 hover:bg-brand-orange/[0.2] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Propose Changes
              </button>
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold bg-[var(--color-status-info)]/[0.12] border border-blue-400/[0.3] text-status-info chamfer-4 hover:bg-[var(--color-status-info)]/[0.2] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                disabled={!activeRun || activeRun.status !== 'approved'}
                  className="px-3 py-1.5 text-xs font-semibold bg-orange-ghost border border-brand-orange/[0.4] text-[var(--q-orange)] chamfer-4 hover:bg-brand-orange/[0.2] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Merge when Green
              </button>
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold bg-[var(--color-bg-base)]/5 border border-white/[0.12] text-[var(--color-text-secondary)] chamfer-4 hover:bg-[var(--color-bg-base)]/[0.09] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Export Patch
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <textarea
                ...
                rows={2}
                className="flex-1 bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder-text-[var(--color-text-muted)] chamfer-4 resize-none focus:outline-none focus:border-orange transition-colors"
              />
              <button
                onClick={sendMessage}
                disabled={sending || !inputText.trim()}
                  className="px-4 bg-[var(--q-orange)] text-black text-xs font-bold chamfer-4 hover:bg-[var(--q-orange)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed self-stretch"
              >
                {sending ? '…' : 'Send'}
              </button>
            </div>
      ...
      {/* Right panel */}
      <aside className="w-96 flex-shrink-0 border-l border-border-subtle flex flex-col bg-[var(--color-bg-base)]">
          ...
          {RIGHT_TABS.map((tab) => (
            <button
              ...
              className={`px-3 py-2.5 text-micro font-semibold uppercase tracking-wider whitespace-nowrap transition-colors flex-shrink-0 ${
                rightTab === tab.id
                  ? 'text-[var(--q-orange)] border-b-2 border-orange'
        ...
        <div className="flex-1 overflow-y-auto">
          {rightTab === 'plan' && (currentPlan ? <PlanPanel plan={currentPlan} /> : (
            <div className="p-4 text-sm text-[var(--color-text-muted)] italic">No plan yet. Ask the copilot to propose a change.</div>
          ))}
              ...
              {currentPlan?.actions.filter((a) => a.type === 'GENERATE_MIGRATION').length ? (
                currentPlan.actions.filter((a) => a.type === 'GENERATE_MIGRATION').map((a, i) => (
                  <div key={i} className="bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT chamfer-4 p-3">
                    ...
                    <pre className="text-micro text-[var(--color-text-secondary)] whitespace-pre-wrap">{JSON.stringify(a.payload, null, 2)}</pre>
                  </div>
                ))
              ) : (
                <div className="text-sm text-[var(--color-text-muted)] italic">No migration actions in this plan.</div>
              )}
            </div>
          )}
              ...
              {currentPlan?.actions.filter((a) => a.type === 'UPDATE_CLOUDFORMATION').length ? (
                currentPlan.actions.filter((a) => a.type === 'UPDATE_CLOUDFORMATION').map((a, i) => (
                  <div key={i} className="bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT chamfer-4 p-3">
                    ...
                    <pre className="text-micro text-[var(--color-text-secondary)] whitespace-pre-wrap">{String((a.payload as Record<string, unknown>).patch || JSON.stringify(a.payload, null, 2))}</pre>
                  </div>
                ))
              ) : (
                <div className="text-sm text-[var(--color-text-muted)] italic">No CloudFormation actions in this plan.</div>
              )}
            </div>
          )}
          ...
          {rightTab === 'notes' && (
            <div className="p-4 space-y-3">
              {activeRun ? (
                <>
                  <div className="text-micro uppercase tracking-widest text-[var(--color-text-muted)] mb-2">Run Details</div>
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between"><span className="text-[var(--color-text-muted)]">Run ID</span><code className="text-[var(--color-text-primary)] font-mono text-micro">{activeRun.id.slice(0, 12)}…</code></div>
                    <div className="flex justify-between"><span className="text-[var(--color-text-muted)]">Status</span><span style={{ color: STATUS_COLORS[activeRun.status] || 'var(--color-text-muted)' }} className="font-semibold">{activeRun.status}</span></div>
                    <div className="flex justify-between"><span className="text-[var(--color-text-muted)]">Created</span><span className="text-[var(--color-text-primary)]">{new Date(activeRun.created_at).toLocaleString()}</span></div>
                  ...
                  {currentPlan?.summary && (
                    <div className="mt-4 p-3 bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT chamfer-4">
                      <div className="text-micro uppercase tracking-widest text-[var(--color-text-muted)] mb-1">Summary</div>
                      <p className="text-xs text-[var(--color-text-primary)]">{currentPlan.summary}</p>
                    </div>
                  )}
                </>
              ) : (
                <div className="text-sm text-[var(--color-text-muted)] italic">No run active.</div>
              )}
            </div>
          )}
        </div>
