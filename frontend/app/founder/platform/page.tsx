    ...
    return (
      <div className="p-8">
        <div className="bg-red-500/10 border border-red-500/40 p-6 chamfer-8">
          ...
          <button onClick={fetchHealth} className="px-4 py-2 bg-red-500/20 border border-red-500/40 text-red-300 text-sm hover:bg-red-500/30 transition-colors chamfer-4">
            Retry
          </button>
        </div>
      </div>
      ...
      {(incidents.length > 0 || (aiAnalysis && aiAnalysis.severity !== 'GREEN')) && (
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-3">
          {incidents.slice(0, 3).map((inc) => {
            ...
            return (
              <div key={inc.id} className="bg-zinc-900 border border-zinc-700 p-3 chamfer-4">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-micro font-mono font-bold ${sev}`}>{inc.severity}</span>
                  <span className="text-micro text-zinc-500">{inc.state}</span>
                </div>
          ...
          {aiAnalysis && aiAnalysis.severity !== 'GREEN' && incidents.length < 3 && (
            <div className="bg-zinc-900 border border-orange-500/30 p-3 chamfer-4">
              <div className="flex items-center gap-2 mb-1">
                <span className={`text-micro font-mono font-bold ${SEVERITY_COLORS[aiAnalysis.severity] || 'text-zinc-400'}`}>
                  {aiAnalysis.severity}
                </span>
                <span className="text-micro text-zinc-500">AI DETECTED</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-zinc-900/60 border border-zinc-800 chamfer-8 p-4">
              ...
              <div className="space-y-2">
                {health.services.map((s) => {
                  ...
                  return (
                    <div key={s.name} className={`flex justify-between items-center ${st.bg} p-2.5 border ${st.border} chamfer-4`}>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${st.dot}`} />
                        <span className="text-xs text-white">{s.name}</span>
                      </div>
                      <span className="text-micro font-mono text-zinc-400">{s.latency_ms}ms &middot; {s.uptime}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="bg-zinc-900/60 border border-zinc-800 chamfer-8 p-4">
              <h3 className="text-micro uppercase tracking-[0.15em] text-zinc-500 mb-3">Integration Health</h3>
              <div className="space-y-2">
                {health.integrations.map((i) => {
                  const st = getStatusStyle(i.status);
                  return (
                    <div key={i.name} className={`flex justify-between items-center ${st.bg} p-2.5 border ${st.border} chamfer-4`}>
                      <span className="text-xs text-white">{i.name}</span>
                      <span className={`text-micro font-mono ${st.text}`}>{i.last_sync.toUpperCase()}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {/* CI/CD & Queue Safety */}
          <div className="bg-zinc-900/60 border border-zinc-800 chamfer-8 p-4">
            <h3 className="text-micro uppercase tracking-[0.15em] text-zinc-500 mb-3">CI/CD &amp; Queue Safety</h3>
            <div className="flex flex-wrap gap-6">
              <div>
                <div className="text-micro text-zinc-500 mb-1 font-mono">LATEST BUILD</div>
                <div className="text-xs text-green-400 font-mono border border-green-500/30 px-2 py-1 bg-green-500/10 chamfer-4">
                  {health.ci_cd.last_build} [{health.ci_cd.branch}]
                </div>
              </div>
              <div>
                <div className="text-micro text-zinc-500 mb-1 font-mono">DEPLOY GATE</div>
                <div className="text-xs text-blue-400 font-mono border border-blue-500/30 px-2 py-1 bg-blue-500/10 chamfer-4">
                  {health.ci_cd.deployment}
                </div>
              </div>
              {health.queues.length > 0 && (
                <div className="flex-1">
                  <div className="text-micro text-zinc-500 mb-1 font-mono">ACTIVE QUEUES</div>
                  <div className="flex flex-wrap gap-2">
                    {health.queues.map((q) => {
                      const st = getStatusStyle(q.status);
                      return (
                        <div key={q.name} className={`text-micro font-mono ${st.bg} px-2 py-1 ${st.text} border ${st.border} chamfer-4`}>
            ...
            {incidents.length === 0 ? (
              ...
              <div className="space-y-2">
                {incidents.map((inc) => {
                  const sev = SEVERITY_COLORS[inc.severity] || 'text-zinc-400';
                  return (
                    <div key={inc.id} className="flex justify-between items-center bg-zinc-800/50 p-2.5 border border-zinc-700 chamfer-4">
                      <div className="flex items-center gap-3">
                        <span className={`text-micro font-mono font-bold ${sev}`}>{inc.severity}</span>
                        <span className="text-xs text-white">{inc.title}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-micro text-zinc-500 font-mono">{inc.state}</span>
                        <span className="text-micro text-zinc-600 font-mono">{new Date(inc.created_at).toLocaleTimeString()}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        <div>
          <div className="bg-zinc-900/60 border border-orange-500/30 chamfer-8 p-5 shadow-[0_0_20px_rgba(255,107,26,0.08)] sticky top-5">
            ...
            {!aiAnalysis ? (
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-micro text-zinc-500 border-b border-zinc-800 mb-1 pb-1">ISSUE &amp; SEVERITY</div>
                  <div className="text-sm text-white font-bold">{aiAnalysis.issue}</div>
                  <div className={`text-micro font-mono mt-1 ${SEVERITY_COLORS[aiAnalysis.severity] || 'text-zinc-400'}`}>
                    {aiAnalysis.severity} &bull; {aiAnalysis.source}
                  </div>
                </div>

                <div>
                  <div className="text-micro text-zinc-500 border-b border-zinc-800 mb-1 pb-1">WHAT IS WRONG</div>
                  <div className="text-xs text-zinc-200">{aiAnalysis.what_is_wrong}</div>
                </div>

                <div>
                  <div className="text-micro text-zinc-500 border-b border-zinc-800 mb-1 pb-1">WHY IT MATTERS</div>
                  <div className="text-xs text-zinc-200">{aiAnalysis.why_it_matters}</div>
                </div>

                <div className="bg-orange-500/10 border border-orange-500/30 p-3 chamfer-4">
                  <div className="text-micro text-orange-400 mb-1 font-bold tracking-wider">WHAT YOU SHOULD DO NEXT</div>
                  <div className="text-xs text-white font-mono">{aiAnalysis.what_to_do_next}</div>
                </div>

                <div>
                  <div className="text-micro text-zinc-500 border-b border-zinc-800 mb-1 pb-1">TECHNICAL CONTEXT</div>
                  <div className="text-xs text-zinc-300 font-mono">{aiAnalysis.tech_context}</div>
                </div>

                <div className="grid grid-cols-2 gap-2 text-micro font-mono pt-3 border-t border-zinc-800">
                  <div>
