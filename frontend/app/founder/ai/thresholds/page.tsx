...
const CONFIDENCE_THRESHOLDS = [
  ...
  { tier: 'HIGH_RISK', label: 'High Risk', threshold: 'HIGH confidence required for auto-approve', color: 'var(--q-orange)' },
  ...
  return (
    <div className="p-5 min-h-screen space-y-6">
      <div>
        ...
        <h1 className="text-xl font-black uppercase tracking-wider text-[var(--color-text-primary)]">Confidence Thresholds</h1>
        <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Escalation rules per risk tier · auto-approve boundaries</p>
      </div>
      ...
      <div className="bg-[var(--color-bg-panel)] border border-border-DEFAULT p-5" style={{ clipPath: 'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 0 100%)' }}>
        <div className="text-micro font-semibold uppercase tracking-widest text-[var(--color-text-muted)] mb-4">Confidence Policy by Risk Tier</div>
        <div className="space-y-2">
          {CONFIDENCE_THRESHOLDS.map((ct) => (
            <div key={ct.tier} className="flex items-center gap-3 py-2 border-b border-white/5 last:border-0">
              ...
              <span className="text-xs text-[var(--color-text-secondary)]">{ct.threshold}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <div className="text-micro font-semibold uppercase tracking-widest text-[var(--color-text-muted)] mb-3">Active Use Cases</div>
        {loading ? (
          ...
          <div className="bg-[var(--color-bg-panel)] border border-border-DEFAULT chamfer-8 p-10 text-center text-xs text-[var(--color-text-muted)]">
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-sm font-bold text-[var(--color-text-primary)]">{uc.name}</span>
                    {!uc.is_enabled && (
