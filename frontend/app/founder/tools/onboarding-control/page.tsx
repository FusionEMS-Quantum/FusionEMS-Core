...
function StatusBadge({ status }: { status: AppStatus }) {
  ...
  return (
    <span
      className={`px-1.5 py-0.5 text-micro font-semibold uppercase tracking-wider chamfer-4 whitespace-nowrap${s.pulse ? ' animate-pulse' : ''}`}
...
function LegalBadge({ legalStatus }: { legalStatus: string }) {
  ...
  return (
    <span
      className="px-1.5 py-0.5 text-micro font-semibold uppercase tracking-wider chamfer-4 whitespace-nowrap"
      style={{ color: s.color, background: s.bg }}
    >
      {s.label}
    </span>
...
function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.6)' }}>
      <div
        className="bg-[var(--color-bg-base)] border border-white/[0.12] chamfer-4 p-5 w-full max-w-sm shadow-[0_0_15px_rgba(0,0,0,0.6)]"
        style={{ boxShadow: '0 0 40px rgba(0,0,0,0.7)' }}
      >
        <h3 className="text-sm font-bold uppercase tracking-wider text-[var(--color-text-primary)] mb-2">{title}</h3>
        <p className="text-body text-[var(--color-text-secondary)] mb-4 leading-relaxed">{message}</p>
        {children && <div className="mb-4">{children}</div>}
        {title.toLowerCase().includes('revoke') && (
          <div className="mb-4">
            <label className="text-micro uppercase tracking-wider text-[var(--color-text-muted)] block mb-1">
              Reason (required)
            </label>
            <input
              value={extra}
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-border-DEFAULT flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-[var(--color-text-primary)] uppercase tracking-wider">Application Detail</h2>
            <p className="text-micro text-[var(--color-text-muted)] mt-0.5">{app.agency_name}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 px-5 py-4 space-y-5">
          {/* Fields */}
          <div>
            <p className="text-[9px] uppercase tracking-[0.18em] text-[var(--q-orange)]/70 mb-3">
              Application Data
            </p>
            <div className="space-y-2.5">
              {fields.map(({ label, value }) => (
                <div key={label} className="flex items-start justify-between gap-3">
                  <span className="text-micro uppercase tracking-wider text-[var(--color-text-muted)] flex-shrink-0 mt-0.5">
                    {label}
                  </span>
                  <span className="text-xs text-[var(--color-text-primary)] text-right font-mono break-all">
                    {value}
                  </span>
                </div>
              ))}
          ...
          <div className="flex items-center justify-between py-3 border-y border-border-subtle">
            ...
            <div className="flex flex-col gap-1.5 items-end">
              <span className="text-micro uppercase tracking-wider text-[var(--color-text-muted)]">Legal</span>
          ...
          <div>
            ...
            {loadingEvents ? (
              <p className="text-body text-[var(--color-text-muted)]">Loading events…</p>
            ) : signEvents.length === 0 ? (
              <p className="text-body text-[var(--color-text-muted)]">No sign events recorded</p>
            ) : (
                  ...
                  >
                    <div className="flex flex-col items-center pt-1">
                      <span
                        ...
                        style={{ background: i === 0 ? 'var(--q-orange)' : 'rgba(255,255,255,0.2)' }}
                      />
                    ...
                    <div className="pb-1">
                      <p className="text-body font-semibold text-[var(--color-text-primary)] uppercase tracking-wide">
                        {ev.event_type}
                      </p>
                      {ev.signer_name && (
                      ...
                      <p className="text-micro font-mono text-[var(--color-text-muted)]">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {app.status === 'paid' && (
                <button
                  ...
                  className="h-7 px-3 text-micro font-semibold uppercase tracking-wider chamfer-4"
                  style={{ background: 'rgba(255,106,0,0.12)', border: '1px solid rgba(255,106,0,0.3)', color: 'var(--q-orange)' }}
                >
                  Manual Provision
                </button>
              )}
    ...
    <div className="min-h-screen bg-[var(--color-bg-base)] text-[var(--color-text-primary)] p-5">
      ...
      {confirm && (
        <ConfirmModal
          ...
          confirmColor={confirm.type === 'provision' ? 'var(--q-orange)' : 'var(--color-brand-red)'}
      ...
      {/* Page header */}
      <div className="mb-5">
        <div className="text-micro font-bold uppercase tracking-[0.2em] text-[var(--q-orange)]/70 mb-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          ...
          <div
            key={s.label}
            className="bg-[var(--color-bg-base)] border border-border-DEFAULT chamfer-4 p-3"
          >
            <div className="text-[9px] uppercase tracking-widest text-[var(--color-text-muted)] mb-1">
              {s.label}
            </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <select
          ...
          className="bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT text-xs text-[var(--color-text-primary)] px-3 py-2 chamfer-4 outline-none focus:border-brand-orange/40"
          style={{ minWidth: 150 }}
        >
          {STATUS_OPTS.map((o) => (
            <option key={o.value} value={o.value} className="bg-[var(--color-bg-base)]">
              {o.label}
            </option>
          ))}
        </select>
        <input
          ...
          className="bg-[var(--color-bg-base)]/[0.04] border border-border-DEFAULT text-xs text-[var(--color-text-primary)] px-3 py-2 chamfer-4 outline-none focus:border-brand-orange/40 placeholder:text-text-disabled"
          style={{ minWidth: 220 }}
        />
      ...
      <div className="bg-[var(--color-bg-base)] border border-border-DEFAULT chamfer-4 overflow-x-auto">
        <table className="w-full text-xs min-w-[860px]">
          <thead>
            <tr className="border-b border-white/[0.07]">
              {[
                ...
                <th
                  key={h}
                  className="text-left py-2.5 px-3 text-[9px] uppercase tracking-wider text-[var(--color-text-muted)] font-semibold whitespace-nowrap"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {loading && (
              <tr>
                <td colSpan={8} className="py-10 text-center text-body text-[var(--color-text-muted)]">
                  Loading…
                </td>
              </tr>
            )}
            ...
            {!loading &&
              apps.map((app) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-3 text-[var(--color-text-secondary)] font-mono">
                    {app.contact_email}
                  </td>
                  <td className="py-2.5 px-3 text-[var(--color-text-secondary)] uppercase">
                    {app.state}
                  </td>
                  <td className="py-2.5 px-3 text-[var(--color-text-secondary)] whitespace-nowrap">
                    {app.plan}
                  </td>
...
function IconBtn({
  ...
  return (
    <button
      onClick={onClick}
      className="w-7 h-7 flex items-center justify-center chamfer-4 text-xs font-bold transition-all hover:brightness-125"
...
function Tooltip({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      {show && (
        <div
          className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2 py-1 chamfer-4 text-[9px] font-semibold uppercase tracking-wider whitespace-nowrap pointer-events-none z-10"
