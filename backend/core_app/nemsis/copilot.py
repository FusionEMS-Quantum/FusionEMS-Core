...
SYSTEM_PROMPT = """You are a NEMSIS (National EMS Information System) v3.5.1 compliance expert for EMS agencies. You explain complex XML/XSD validation errors in plain English and generate structured remediation actions. Always respond with valid JSON only, no markdown.
...
class NEMSISCopilot:
    ...
    def explain_issues(
        ...
        if not issues:
            return {
                "summary": "No validation issues found. Your data is NEMSIS v3.5.1 compliant.",
                "actions": [],
                "confidence": 1.0,
            }
        ...
        version = ctx.get("version", "3.5.1")

        user_prompt = f"""NEMSIS validation issues (state={state}, version={version}):
